...
structdef   : TOK_STRUCT TOK_IDENT '{' identdecls '}' {freeast2($3,$5); 
...
paramlist   : '(' identdecl                 {adopt1sym($1,$2,
                                                       TOK_PARAMLIST);}
            | paramlist ',' identdecl        {freeast($2); 
                                              adopt1($1, $3); }
            | '('                            {adoptsym($1, 
                                                       TOK_PARAMLIST);
                                              $$ = $1; }
...
identdecls  : identdecl ';'                  {freeast($2); $$ = $1;  }
            | identdecls identdecl ';'       {freeast($3); 
                                              $$ = adopt1($1, $2) ; }
            ;
...
vardeclinit : identdecl '=' expr ';' { freeast($4); 
                                       $$ = adopt2($2, $1, $3) ; }
            ;

whilehead   : TOK_WHILE '(' expr ')' statement {freeast2($2, $4); 
                                                $$ = adopt2($1,$3,$5);}
            ;

ifelse      : ifhead statement TOK_ELSE statement {freeast($3); 
                                                 $$ = adopt2($1,$2,$4);}
            | ifhead statement %prec TOK_ELSE     {$$ = adopt1($1,$2);}

ifhead      : TOK_IF '(' expr ')'                 { freeast2($2,$4); 
                                                   $$ = adopt1($1,$3);}
            ;

returnhead  : TOK_RETURN ';'        {$$ = adopt1sym($1,$2,
                                                    TOK_RETURNVOID); }
            | TOK_RETURN expr ';'   { freeast($3);  
                                      $$ = adopt1($1, $2) ; }
            ;
...
allocator   : TOK_NEW TOK_IDENT '('')' { freeast2($3,$4); 
                                         $2 = adoptsym($2,TOK_TYPEID); 
                                         $$ = adopt1($1, $2) ;}
            | TOK_NEW TOK_STRING '(' expr ')' { freeast2($3,$5); 
                                                $$ = adopt1sym($1,$4,
                                                        TOK_NEWSTRING);}
            | TOK_NEW basetype '[' expr ']' { freeast2($3,$5); 
                                              $$ = adopt2sym($1,$2,$4,
                                                        TOK_NEWARRAY); }
            ;

call        : TOK_IDENT call_list ')' { freeast($3); 
                                        $$ = adopt1sym($2, $1, 
                                                       TOK_CALL); }
            | TOK_IDENT '(' ')'       { freeast($3); 
                                        $$ = adopt1sym($2, $1, 
...
call_list   : '(' expr                 { $$ = adopt1($1,$2); }
            |  call_list ',' expr      { freeast($2);
                                         $$ = adopt1($1, $3) ; }
            | '('                      { $$ = $1; }
            ;

variable    : TOK_IDENT          { $$ = $1; }
            | expr '[' expr ']'  { freeast2($2, $4); 
                                   $$ = adopt1($1, $3) ; }
            | expr '.'           { freeast($2); $$ = $1; }
            | TOK_FIELD          { $$ = $1; }
            ;
